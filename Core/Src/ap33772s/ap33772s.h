...
#define AP_ADDRESS (0x52 << 1)

/** Maximum number of Power Data Objects (PDOs) supported (7 SPR + 6 EPR) */
#define AP_MAX_PDO_ENTRIES 13

/** Buffer length for I2C read operations */
#define AP_READ_BUFFER_LENGTH 128

/** Buffer length for I2C write operations */
#define AP_WRITE_BUFFER_LENGTH 6

/** Byte length of all source PDO data (13 PDOs * 2 bytes each) */
#define AP_SRCPDO_BYTE_LEN (AP_MAX_PDO_ENTRIES * 2)

/** CONFIG register bit definitions */
#define AP_CONFIG_DR_EN   (1u << 7)
#define AP_CONFIG_OTP_EN  (1u << 6)
#define AP_CONFIG_OCP_EN  (1u << 5)
#define AP_CONFIG_OVP_EN  (1u << 4)
#define AP_CONFIG_UVP_EN  (1u << 3)

/** PDO supply types reported by the AP33772S */
enum ap_pdo_supply {
    AP_PDO_SUPPLY_FIXED = 0,
    AP_PDO_SUPPLY_PPS,
    AP_PDO_SUPPLY_AVS,
};
...
struct ap_pdo_info {
    ...
    enum ap_pdo_supply supply; /**< Reported supply type */
 ...
 */
enum ap_mask {
    AP_MASK_STARTED = 1 << 0,  /**< System started, ready for configuration */
    AP_MASK_READY   = 1 << 1,  /**< Ready to receive I2C requests/commands */
    AP_MASK_NEWPDO  = 1 << 2,  /**< New source PDOs received */
    AP_MASK_UVP     = 1 << 3,  /**< Undervoltage protection triggered */
    AP_MASK_OVP     = 1 << 4,  /**< Overvoltage protection triggered */
    AP_MASK_OCP     = 1 << 5,  /**< Overcurrent protection triggered */
    AP_MASK_OTP     = 1 << 6   /**< Overtemperature protection triggered */
};

struct ap33772s;

/** Opaque reference to an AP33772S device instance */
typedef struct ap33772s *ap_ref;

struct ap_bus_delegate {
 ...
 * @return 0 on success, negative errno on failure
 */
int a_read_dr_threshold(ap_ref dev, int *temp_c);
 ...
 * @return 0 on success, negative errno on failure
 */
int ap_send_hard_reset(ap_ref dev);
